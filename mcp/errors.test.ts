...
// NOTE: the 'invalid_type_args' suite was removed in the CF-only migration — its
// only case relied on the generic Delay program (retired). With no generic stdlib
// type, the bad-type_args path is unreachable from the stdlib (the test had
// already degenerated to an unknown_program check). The invalid_type_args error
// code is still live in the engine. (doc-sweep tracked)
