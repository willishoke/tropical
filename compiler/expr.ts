...
// ---------- Compile-time combinator constructors ----------

/** Lexically-scoped variable reference, resolved during combinator expansion. */
export function bindingExpr(name: string): SignalExpr {
  return SignalExpr.fromNode({ op: 'binding', name })
}

/** Named bindings with sequential let* semantics. */
export function let_(bindings: Record<string, ExprCoercible>, body: ExprCoercible): SignalExpr {
  const bindNodes: Record<string, ExprNode> = {}
  for (const [k, v] of Object.entries(bindings)) bindNodes[k] = coerce(v)._node
  return SignalExpr.fromNode({ op: 'let', bind: bindNodes, in: coerce(body)._node })
}

/** Build an array of `count` elements from an index variable. */
export function generate(count: number, varName: string, body: ExprCoercible): SignalExpr {
  return SignalExpr.fromNode({ op: 'generate', count, var: varName, body: coerce(body)._node }, [count])
}

/** Constant-fill array (sugar over generate with unused variable). */
export function repeat(count: number, value: ExprCoercible): SignalExpr {
  return SignalExpr.fromNode({ op: 'generate', count, var: '_', body: coerce(value)._node }, [count])
}

/** Produce [init, f(init), f(f(init)), ...] of length `count`. */
export function iterate(count: number, init: ExprCoercible, varName: string, body: ExprCoercible): SignalExpr {
  return SignalExpr.fromNode(
    { op: 'iterate', count, init: coerce(init)._node, var: varName, body: coerce(body)._node },
    [count],
  )
}

/** Left fold over an array with arbitrary body expression. */
export function fold(
  over: ExprCoercible, init: ExprCoercible,
  accVar: string, elemVar: string, body: ExprCoercible,
): SignalExpr {
  return SignalExpr.fromNode({
    op: 'fold', over: coerce(over)._node, init: coerce(init)._node,
    acc_var: accVar, elem_var: elemVar, body: coerce(body)._node,
  })
}

/** Fold keeping intermediates — returns array of length equal to input. */
export function scan(
  over: ExprCoercible, init: ExprCoercible,
  accVar: string, elemVar: string, body: ExprCoercible,
): SignalExpr {
  return SignalExpr.fromNode({
    op: 'scan', over: coerce(over)._node, init: coerce(init)._node,
    acc_var: accVar, elem_var: elemVar, body: coerce(body)._node,
  })
}

/** Pointwise transform with named element variable. Generalizes mapArray. */
export function map2(over: ExprCoercible, elemVar: string, body: ExprCoercible): SignalExpr {
  return SignalExpr.fromNode({
    op: 'map2', over: coerce(over)._node, elem_var: elemVar, body: coerce(body)._node,
  })
}

/** Combine two arrays pointwise with named element variables. */
export function zipWith(
  a: ExprCoercible, b: ExprCoercible,
  xVar: string, yVar: string, body: ExprCoercible,
): SignalExpr {
  return SignalExpr.fromNode({
    op: 'zip_with', a: coerce(a)._node, b: coerce(b)._node,
    x_var: xVar, y_var: yVar, body: coerce(body)._node,
  })
}

/** Serial pipeline: apply body `count` times, threading result. */
export function chain(count: number, init: ExprCoercible, varName: string, body: ExprCoercible): SignalExpr {
  return SignalExpr.fromNode({
