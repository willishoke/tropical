...
def obligations : Array Obligation := #[
  ...
  { id := "METAL_CALLBACK_CONSUMES_PREPARED_EPOCHS"
    ...
    implementationPaths := #["engine/runtime/EpochTileQueue.hpp", "engine/metal/MetalRenderWorker.cpp", "engine/runtime/FlatRuntime.hpp", "engine/metal/MetalKernel.mm"]
    gateNames := #["metal-ctest", "manual:Metal callback source review"]
    owner := "Metal runtime"
    status := .evidenceBacked
  ...
  { id := "HOST_PARAM_DISCIPLINES"
    statement := "Lean and socket hosts apply the same re-anchoring formulas and slot order; Metal recomputes the whole transaction at its exact audible activation epoch E."
    evidence := #[.differential, .executableGate, .inspection]
    implementationPaths := #["lean/Tropical/Engine", "engine/runtime/FlatRuntime.cpp", "engine/runtime/FlatRuntime.hpp", "mcp"]
    gateNames := #["mcp-protocol", "metal-ctest", "manual:host re-anchor inspection"]
