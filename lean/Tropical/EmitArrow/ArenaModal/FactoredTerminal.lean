import Tropical.EmitArrow.ArenaModal.OrientedRealize
...
private def FactoredSlice.read (slice : FactoredSlice) (image : Sig)
  ...
  let real ← Tropical.EmitArrow.ArenaNative.index image realIndex
  let imagIndex ← lit (Int.ofNat (slice.offset + 2 * index + 1))
  let imag ← Tropical.EmitArrow.ArenaNative.index image imagIndex
  pure (real, imag)
...
private def SourceRoomImage.readSource (image : SourceRoomImage)
  ...
  image.parts.foldlM (fun total part => do
    ...
    let real ← Tropical.EmitArrow.ArenaNative.index part.image realIndex
    let imagOffset ← lit (Int.ofNat (selected.offset + 1))
    let imagIndex ← add imagOffset doubled
    let imag ← Tropical.EmitArrow.ArenaNative.index part.image imagIndex
...
private def ModeColumns.readPole (columns : ModeColumns) (itemIndex : Sig) : BuildM CplxE := do
  let real ← Tropical.EmitArrow.ArenaNative.index columns.poleRe itemIndex
  let imag ← Tropical.EmitArrow.ArenaNative.index columns.poleIm itemIndex
  pure (real, imag)

private def ModeColumns.readAmp (columns : ModeColumns) (itemIndex : Sig) : BuildM CplxE := do
  let real ← Tropical.EmitArrow.ArenaNative.index columns.ampRe itemIndex
  let imag ← Tropical.EmitArrow.ArenaNative.index columns.ampIm itemIndex
  pure (real, imag)
...
private def RoomPairImage.read (image : RoomPairImage)
  ...
  image.images.foldlM (fun total part => do
    ...
    let realOffset ← lit (Int.ofNat selected.offset)
    let realIndex ← add realOffset doubled
    let real ← Tropical.EmitArrow.ArenaNative.index part realIndex
    let imagOffset ← lit (Int.ofNat (selected.offset + 1))
    let imagIndex ← add imagOffset doubled
    let imag ← Tropical.EmitArrow.ArenaNative.index part imagIndex
    caddE total (real, imag)) zero
...
private def readScalarParts (images : Array Sig) (offset : Nat)
  ...
  images.foldlM (fun total image => do
    ...
    let value ← Tropical.EmitArrow.ArenaNative.index image scalarIndex
...
private def routedSourceSideSig (factored : FactoredTwoRoomTerminal)
  ...
  let sideResult ← Tropical.EmitArrow.ArenaNative.index image zero
  let one ← lit 1
  let strikeResult ← Tropical.EmitArrow.ArenaNative.index image one
...
private def routedColdSourceSideSig (factored : FactoredTwoRoomTerminal)
  ...
  let result ← Tropical.EmitArrow.ArenaNative.index image zero
  pure (result, zero)
...
private def liveModePole (image itemIndex : Sig) : BuildM CplxE := do
  ...
  let real ← Tropical.EmitArrow.ArenaNative.index image base
  let one ← lit 1
...
private def routedInlineColdSourceSideSig (modes : Array ModalMode)
  ...
  let image ← routedSum capacity 1 (sumRoutes capacity)
    #[modeImage, assembly]
    #[value] none 15
  Tropical.EmitArrow.ArenaNative.index image zero
