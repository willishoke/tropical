...
private def sigConstTable (arena : ExprArena) : Array (Option DyadicI) := Id.run do
  let mut values : Array (Option DyadicI) := #[]
...
private def sigConstDFrom? (constants : Array (Option DyadicI))
...
private def settleSignals (roots : Array Sig) : BuildM (Option (Array Sig)) := do
  let source ← get
