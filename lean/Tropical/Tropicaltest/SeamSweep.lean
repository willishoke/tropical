...
def SeamMode.toModal (m : SeamMode) : BuildM ModalMode := do
  let sigma ← litF m.sigma
  let omega ← litF m.omega
  let cre ← litF m.are
  let cim ← litF m.aim
  pure { sigma, omega, cre, cim }
def SeamMode.pole (m : SeamMode) : CplxB := ⟨-m.sigma, m.omega⟩
def SeamMode.amp  (m : SeamMode) : CplxB := ⟨m.are, m.aim⟩
def SeamMode.dutPole (m : SeamMode) : Tropical.EmitArrow.CplxB :=
  ⟨-m.sigma, m.omega⟩
  ...
  for v in voice do
    ...
    for r in reverb do
      ...
      for i in [anchorNat + 1 : nLen] do
        ...
        let yc := (Tropical.Testing.ArrowOracles.CplxB.exp (nu.scale d)).mul jAcc
...
structure SeamAtom where
  ...
  realize : Array SeamMode → Array SeamMode → BuildM Sig
...
/-- Independent Float restatement of the baked degree-zero admission cap.  The
    DUT's routing decision is checked separately on its frozen arena; this pure
    predicate only labels oracle boundary samples. -/
private def composeAdmitsPair (v r : SeamMode) : Bool :=
  let dSigma := v.sigma - r.sigma
  let dOmega := v.omega - r.omega
  let delta := Float.sqrt (dSigma * dSigma + dOmega * dOmega)
  let coupling := Float.sqrt (v.are * v.are + v.aim * v.aim) *
    Float.sqrt (r.are * r.are + r.aim * r.aim)
  let lens := delta < 0.4642 || (delta > 0.0 && coupling / delta > 4.0)
  let cap := (delta > 0.0 && 2.0 * coupling / delta < 8.0) ||
    (min v.sigma r.sigma > 0.0 &&
...
private def composeAtom : SeamAtom :=
  { name := "residueCompose"
    ...
    realize := fun v r => do
      let (plain, paired) ←
        residueComposePartitioned (← v.mapM (·.toModal)) (← r.mapM (·.toModal))
      let clock ← clockLit
      let anchor ← anchorSig
      let base ← modalBankSigTable plain clock anchor
      if paired.isEmpty then pure base
      else add base (← modalBankSigTableDD paired clock anchor)
...
private def bloomAtom : SeamAtom :=
  ...
  { name := "bloomGamma"
    phi := bloomWarp
    realize := fun v r => do
      match ← bloomCompose (← v.mapM (·.toModal)) (← r.mapM (·.toModal)) B g with
      | some pairs => bloomComposedSig pairs (← clockLit) (← anchorSig)
      | none => lit 0
    admitsPair := fun v r => bloomAdmitsPair v.dutPole r.dutPole B g
...
private def renderConfig (arena : Arena) (name : String) (sig : BuildM Sig)
    (nLen : Nat := nProbe) : IO (Except String (Array Float)) := do
  match buildAndFinish (buildExprCarrier name sig arena) with
...
def runSeamSweep (arena : Arena)
  ...
  let ecFwd := fun (x y : Array SeamMode) => do
    let modes ← residueComposeEC (← x.mapM (·.toModal)) (← y.mapM (·.toModal))
    modalBankSigTable modes (← clockLit) (← anchorSig)
  ...
  let left : BuildM Sig := do
    let leftModes ← residueComposeEC
      (← residueComposeEC (← vA.mapM (·.toModal)) (← rA.mapM (·.toModal)))
      (← rB.mapM (·.toModal))
    modalBankSigTable leftModes (← clockLit) (← anchorSig)
  let right : BuildM Sig := do
    let roomModes ← residueComposeEC (← rA.mapM (·.toModal)) (← rB.mapM (·.toModal))
    let rightModes ← residueComposeEC (← vA.mapM (·.toModal)) roomModes
    modalBankSigTable rightModes (← clockLit) (← anchorSig)
  match ← renderConfig arena "seam_law_assoc_l" left,
        ← renderConfig arena "seam_law_assoc_r" right with
  | .ok dl, .ok dr =>
  ...
  let (B, g) := bloomBg
  let admD := fun (x y : SeamMode) => bloomAdmitsPair x.dutPole y.dutPole B g
  ...
  let admS := fun (x y : SeamMode) => bloomAdmitsPair x.dutPole y.dutPole bSmall g
  ...
  let realizeSmall : BuildM Sig := do
    match ← bloomCompose (← vSmall.mapM (·.toModal)) (← rSmall.mapM (·.toModal)) bSmall g with
    | some pairs => bloomComposedSig pairs (← clockLit) (← anchorSig)
    | none => lit 0
...
private def renderGraph (arena : Arena) (name : String) (gr : BuildM PatchGraph) :
    IO (Except String (Array Float)) :=
  renderConfig arena name do
    let term ← lowerGraph (← gr)
...
private def gongVerdict (voice room1 room2 : Array SeamMode)
  ...
  let hi := nProbe
  let (B, g) := bloomBg
  let adm := fun (v r : SeamMode) => bloomAdmitsPair v.dutPole r.dutPole B g
...
def runGongReverb (arena : Arena)
  ...
  let gSingle : BuildM PatchGraph := do
    let vM ← voice.mapM (·.toModal)
    let r1M ← room1.mapM (·.toModal)
    let clk ← clockLit
    let anchor ← anchorSig
    let nodes : Array PatchNode := #[
      { id := "src", node := .modalSource vM anchor clk none none (some (B, g)) },
      { id := "rev", node := .modalReverb "src" r1M none }]
    pure { nodes, output := "rev" }
  let gChain : BuildM PatchGraph := do
    let vM ← voice.mapM (·.toModal)
    let r1M ← room1.mapM (·.toModal)
    let r2M ← room2.mapM (·.toModal)
    let clk ← clockLit
    let anchor ← anchorSig
    let nodes : Array PatchNode := #[
      { id := "src", node := .modalSource vM anchor clk none none (some (B, g)) },
    ...
    pure { nodes, output := "rev2" }
  -- the direct atoms the lowering should route through (single + folded chain)
  let directSingle : BuildM Sig := do
    let some pairs ← bloomCompose (← voice.mapM (·.toModal))
        (← room1.mapM (·.toModal)) B g | lit 0
    bloomComposedSig pairs (← clockLit) (← anchorSig)
  let directChain : BuildM Sig := do
    let folded ← residueComposeEC (← room1.mapM (·.toModal))
      (← room2.mapM (·.toModal))
    let some pairs ← bloomCompose (← voice.mapM (·.toModal)) folded B g | lit 0
    bloomComposedSig pairs (← clockLit) (← anchorSig)
...
private def liveRoomAt (rtLo rtHi rv : Float) (room : Array SeamMode) :
    BuildM (Array ModalMode) :=
  room.mapM fun mode => do
    let base ← mode.toModal
    let numerator ← lit 691 2
    let value ← litF rv
    let lo ← litF rtLo
    let hi ← litF rtHi
    let sigma ← div numerator (← clampE value lo hi)
    pure { base with sigma, sigmaRange := some (6.91 / rtHi, 6.91 / rtLo) }

private def bloomSignal (voice room : Array SeamMode) (B g : Float)
    (liveRt60? : Option (Float × Float × Float) := none) : BuildM Sig := do
  let voice ← voice.mapM (·.toModal)
  let room ← match liveRt60? with
    | some (lo, hi, value) => liveRoomAt lo hi value room
    | none => room.mapM (·.toModal)
  let some pairs ← bloomCompose voice room B g | lit 0
  bloomComposedSig pairs (← clockLit) (← anchorSig)

private def bloomGraph (voice room : Array SeamMode) (B g : Float)
    (liveRt60? : Option (Float × Float × Float) := none) : BuildM PatchGraph := do
  let voice ← voice.mapM (·.toModal)
  let room ← match liveRt60? with
    | some (lo, hi, value) => liveRoomAt lo hi value room
    | none => room.mapM (·.toModal)
  ...
  let clock ← clockLit
  pure { nodes := #[
    { id := "src", node := .modalSource voice anchor clock none none (some (B, g)) },
    { id := "rev", node := .modalReverb "src" room none }], output := "rev" }
...
def runBloomLivePole (arena : Arena)
  ...
  -- Inspect IDs only together with the frozen arena that owns them.  Rendering
  -- below rebuilds each probe atomically from its source data.
  let inspected := freezeBuild arena do
    let vM ← voice.mapM (·.toModal)
    let liveRoom ← liveRoomAt rtLo rtHi rt60Val room
    let some pairsLive ← bloomCompose vM liveRoom B g
      | throw "bloomCompose explicitly refused the supported live-rt60 room"
    let near ← bloomComposeChecked vM
      (← liveRoomAt rtLo rtHi rt60Val #[mkMode 220.05 sigLive 0.5]) B g
    let nearRefused := !near.isComplete && near.pairs.isEmpty &&
      near.exclusions.any (fun x =>
        x.reason == .liveRegionUnsupported || x.reason == .excludedConditioning)
    let straddle ← bloomCompose vM
      (← liveRoomAt rtLo rtHi rt60Val #[mkMode 230.5 sigLive 0.5]) B g
    let xPairs ← bloomCompose (← voiceX.mapM (·.toModal))
      (← liveRoomAt rtLo rtHi rt60Val roomX) B g
    pure (pairsLive, nearRefused, straddle.map (·.size == 3),
      xPairs.map fun pairs => pairs.size == 1 && pairs.all (fun p => !p.cfN.isEmpty))
  let (frozen, pairsLive, nearRefused, straddleBankFull, crossingLifted) ←
    match inspected with
    | .error error =>
        return ← failGate "bloom-live-pole" s!"arena-native inspection: {firstLine error}"
    | .ok (frozen, (pairs, nearRefused, full?, lifted?)) =>
        pure (frozen, pairs, nearRefused, full?.getD false, lifted?.getD false)
  ...
  let s0 := sigS0Table frozen
  let genuinelyLive := pairsLive.all fun p =>
    (sigConstDFrom? constants p.nuSigma).isNone
  let allS0 := liveConsts.all (sigIsS0From s0)
  ...
  let straddleCheck : Float → String → IO (Option (Float × Float)) := fun rv tag => do
    ...
    let shape := freezeBuild arena do
      let some pairs ← bloomCompose (← vS.mapM (·.toModal))
          (← liveRoomAt rtLo rtHi rv rS) B g | return false
      pure (pairs.size == 1 && pairs.all fun p => !p.cfN.isEmpty)
    let .ok (_, true) := shape | return none
    match ← renderConfig arena s!"wslp_str_{tag}_l"
          (bloomSignal vS rS B g (some (rtLo, rtHi, rv))),
        ← renderConfig arena s!"wslp_str_{tag}_b" (bloomSignal vS rS B g) with
    | .ok l, .ok b =>
      let admS := fun (v r : SeamMode) => bloomAdmitsPair v.dutPole r.dutPole B g
  ...
  let rLive ← renderGraph arena "wslp_live"
    (bloomGraph voice room B g (some (rtLo, rtHi, rt60Val)))
  let rBaked ← renderGraph arena "wslp_baked" (bloomGraph voice room B g)
  let rLiveX ← renderConfig arena "wslp_xlive"
    (bloomSignal voiceX roomX B g (some (rtLo, rtHi, rt60Val))) nX
  let rBakedX ← renderConfig arena "wslp_xbaked" (bloomSignal voiceX roomX B g) nX
...
private def psExp (n : Nat) (p : PS) : PS := Id.run do
  let mut e := psZero n
  e := e.set! 0 (Tropical.Testing.ArrowOracles.CplxB.exp p[0]!)
  for m in [1:n + 1] do
    let mut s : CplxB := ⟨0, 0⟩
    for k in [1:m + 1] do
...
def runGammaCoeff (_arena : Arena)
  ...
  for jr in [0:n] do
    let j := n - 1 - jr
    let coef := Tropical.Testing.ArrowOracles.CplxB.div ⟨1, 0⟩
      (aC.add ⟨(j + 1).toFloat, 0⟩)
    Mz := psAdd n (psConst n ⟨1, 0⟩) (psMul n (psScaleC coef z) Mz)
  let Mkappa := (Tropical.Testing.ArrowOracles.bloomM1 aC kappa).1
  -- E1 closed form F(d)
  let num := psSub n (psScaleC Mkappa eNu) (psMul n Mz eMuPhi)
  let fSeries := psScaleC
    (Tropical.Testing.ArrowOracles.CplxB.div ⟨1, 0⟩ (nu.sub mu)) num
...
private def tallyBatch (count : Nat) (B g : Float) (near : Bool) : RegionTally := Id.run do
  ...
  for i in [1:count + 1] do
    ...
    let plan := classifyBloomPair (dutCplx mu) (dutCplx nu) B g
...
def runBloomSafety (arena : Arena)
  ...
  let planAt := fun (reA : Float) (b : Float) =>
    ...
    classifyBloomPair (dutCplx mu) (dutCplx nu) b g
  let hazard := planAt (-0.98) B
  let metric := Tropical.Testing.ArrowOracles.bloomConditioningMetric
    ⟨hazard.aC.re, hazard.aC.im⟩ bloomConditioningLatticeDepth
  ...
  let equalityRefused := bloomExcludedConditioningD
    (dutCplx (⟨-0.984375, 0⟩ : CplxB)).toExact
    (dutCplx (⟨0.015625, 0⟩ : CplxB)).toExact
  ...
  let census ← match freezeBuild arena do
      let (full, half) ← defaultGongModes 110.0
      let room ← Tropical.Playground.Compiler.bakedReverbProbe 32
      pure (full, half, room) with
    | .error error =>
        return ← failGate "bloom-safety" s!"default coefficient probe: {firstLine error}"
    | .ok (frozen, tables) => pure (frozen, tables)
  let (frozen, full, half, room) := (census.1, census.2.1, census.2.2.1, census.2.2.2)
  let constants := sigConstTable frozen
  let constF := fun signal => (sigConstDFrom? constants signal).map (·.toFloat)
  ...
  for vb in #[(full, B), (half, B * 0.5)] do
    for v in vb.1 do
      let some vs := constF v.sigma | liveExcluded := liveExcluded + room.size; continue
      let some vo := constF v.omega | liveExcluded := liveExcluded + room.size; continue
      let vmu : Tropical.EmitArrow.CplxB := ⟨-vs, vo⟩
      for r in room do
        let some ro := constF r.omega | liveExcluded := liveExcluded + 1; continue
  ...
  let mkModePole := fun (pole : CplxB) => do
    let sigma ← litF (-pole.re)
    let omega ← litF pole.im
    let cre ← lit 1
    let cim ← lit 0
    pure ({ sigma, omega, cre, cim } : ModalMode)
  let structural := freezeBuild arena do
    let unsafeVoice ← mkModePole mu
    let safeVoice ← (mkMode 610.0 1.0 0.5).toModal
    let unsafeRoom ← mkModePole unsafeNu
    let voices := #[unsafeVoice, safeVoice]
    let rooms := #[unsafeRoom]
    let comp ← bloomComposeChecked voices rooms B g
    let safePlan := classifyBloomPair (dutCplx safeMu) (dutCplx unsafeNu) B g
    let mixedRefusal := !comp.isComplete && comp.pairs.isEmpty && comp.expectedPairs == 2
      ...
      && comp.exclusions.size == 1 && match comp.exclusions[0]? with
        | some x => x.voiceIndex == some 0 && x.roomIndex == some 0 &&
          x.reason == .excludedConditioning
        | none => false
    let anchor ← anchorSig
    let clock ← clockLit
    let crossed : PatchGraph := { nodes := #[
      { id := "src", node := .modalSource voices anchor clock none none (some (B, g)) },
      { id := "rev", node := .modalReverb "src" rooms none }], output := "rev" }
    let bareNodes : Array PatchNode := #[
      { id := "src", node := .modalSource voices anchor clock none none (some (B, g)) }]
    let bare : PatchGraph := { nodes := bareNodes, output := "src" }
    let builder ← get
    let patchRefused := match (lowerGraph crossed).run builder with
      | .error error => decide ((error.splitOn "excludedConditioning").length > 1)
      | .ok _ => false
    let bareOk := match (lowerGraph bare).run builder with
      | .ok _ => true
      | .error _ => false
    let muMode ← mkModePole mu
    let k0Mode ← mkModePole k0Nu
    let k0Comp ← bloomComposeChecked #[muMode] #[k0Mode] 0.0 g
    let k0LivePlan := classifyBloomPairLiveChecked (dutCplx mu) mu.im
      (-k0Nu.re) (-k0Nu.re) 0.0 g
    let k0Typed := k0Comp.isComplete && k0Comp.pairs.size == 1 && match k0LivePlan with
      | .ok { region := .serOnly, nDepth, kDepth } => nDepth == 0 && kDepth == 0
      | _ => false
    let depthVoice ← mkModePole depthMu
    let depthRoom ← mkModePole depthNu
    let depthComp ← bloomComposeChecked #[depthVoice] #[depthRoom] depthB g
    let depthTyped := !depthComp.isComplete && depthComp.pairs.isEmpty &&
      depthComp.exclusions.size == 1 && match depthComp.exclusions[0]? with
        | some x => x.voiceIndex == some 0 && x.roomIndex == some 0 &&
          x.reason == .excludedDepth
        | none => false
    let depthGraph : PatchGraph := { nodes := #[
      { id := "src", node := .modalSource #[depthVoice] anchor clock none none
          (some (depthB, g)) },
      { id := "rev", node := .modalReverb "src" #[depthRoom] none }], output := "rev" }
    let depthBuilder ← get
    let depthPatch := match (lowerGraph depthGraph).run depthBuilder with
      | .error error => decide ((error.splitOn "excludedDepth").length > 1)
      | .ok _ => false
    pure (mixedRefusal, patchRefused, bareOk, k0Typed, depthTyped, depthPatch)
  let (mixedRefusal, patchRefused, bareOk, k0Typed, depthTyped, depthPatch) ←
    match structural with
    | .error error =>
        return ← failGate "bloom-safety" s!"arena-native structural probe: {firstLine error}"
    | .ok (_, checks) => pure checks

  -- Exact κ=0, a=-1: empty Horner identity, complete composition, finite render.
  let k0Render ← renderConfig arena "bloom_kappa_zero" do
    let comp ← bloomComposeChecked #[← mkModePole mu] #[← mkModePole k0Nu] 0.0 g
    let some pairs := comp.toOption | throw "κ=0 composition refused"
    bloomComposedSig pairs (← clockLit) (← anchorSig)
  let k0Ref ← renderConfig arena "bloom_kappa_zero_ref" do
    let modes ← residueComposeEC #[← mkModePole mu] #[← mkModePole k0Nu]
    modalBankSig modes (← clockLit) (← anchorSig)
...
private def withCfLane (p : BloomPair) : BuildM BloomPair := do
  ...
  let cf := fun signal => do pure ((← sigConstF? signal).getD 0.0)
  let mu : CplxB := ⟨-(← cf p.muSigma), ← cf p.muOmega⟩
  let nu : CplxB := ⟨-(← cf p.nuSigma), ← cf p.nuOmega⟩
  let aC := (nu.sub mu).scale (1.0 / p.gRate)
  let plan := classifyBloomPair (dutCplx mu) (dutCplx nu) p.bloomB p.gRate
  let kappaB : CplxB := ⟨← cf p.kappa.1, ← cf p.kappa.2⟩
  let (cfK, _) := Tropical.Testing.ArrowOracles.bloomCF aC kappaB
  ...
  let invA := (Array.range plan.nDepth).map (fun k =>
    Tropical.Testing.ArrowOracles.CplxB.div ⟨1, 0⟩
      (aC.add ⟨(k + 1).toFloat, 0⟩))
  let k1Cf ← cplxLitE (dutCplx ((cfK.scale (1.0 / p.gRate)).neg))
  let cfB ← cfB.mapM (cplxLitE ∘ dutCplx)
  let cfN ← cfN.mapM (cplxLitE ∘ dutCplx)
  let invA ← invA.mapM (cplxLitE ∘ dutCplx)
  pure { p with k1Cf, cfB, cfN, invA }
...
def runSeamLaneClean (arena : Arena)
  ...
  let newSig : BuildM Sig := bloomSignal v r bSmall g
  let oldSig : BuildM Sig := do
    let some pairs ← bloomCompose (← v.mapM (·.toModal)) (← r.mapM (·.toModal))
        bSmall g | lit 0
    bloomComposedSig (← pairs.mapM withCfLane) (← clockLit) (← anchorSig)
...
private def oracleFoldChain (voice : Array SeamMode) (nu1p nu2p r1r2 : CplxB)
  ...
  for v in voice do
    ...
    let fint := fun (nu : CplxB) (s : Float) =>
      Tropical.Testing.ArrowOracles.CplxB.exp
        ((mu.scale (s + B * (1.0 - Float.exp (-g * s)))).sub (nu.scale s))
    ...
    for i in [anchorNat + 1 : nLen] do
      ...
      let d := (i - anchorNat).toFloat / srF
      let yc := (((Tropical.Testing.ArrowOracles.CplxB.exp (nu1.scale d)).mul j1).sub
        ((Tropical.Testing.ArrowOracles.CplxB.exp (nu2.scale d)).mul j2)).div dlt
...
def runFoldChain (arena : Arena)
  ...
  let runArm : String → SeamMode → IO (Option (Float × Float × Bool)) := fun tag r2m => do
    ...
    let dut : BuildM Sig := do
      let some pairs ← bloomFoldCompose (← voice.mapM (·.toModal))
          (← r1.toModal) (← r2m.toModal) B g | lit 0
      bloomFoldComposedSig pairs (← clockLit) (← anchorSig)
  ...
  let coll : BuildM Sig := do
    let folded ← residueComposeEC #[← r1.toModal] #[← r2.toModal]
    let some pairs ← bloomCompose (← voice.mapM (·.toModal)) folded B g | lit 0
    bloomComposedSig pairs (← clockLit) (← anchorSig)
...
private def renderTerm (arena : Arena) (name : String) (term : BuildM ArrowTerm)
    (nLen : Nat := nProbe) : IO (Except String (Array Float)) :=
  renderConfig arena name (do emitTerm (normalize (← term))) nLen

private def renderGraphN (arena : Arena) (name : String) (graph : BuildM PatchGraph)
    (nLen : Nat) : IO (Except String (Array Float)) :=
  renderTerm arena name (do lowerGraph (← graph)) nLen

private def modalChainGraph (voice : Array SeamMode)
    (rooms : Array (Array SeamMode)) : BuildM PatchGraph := do
  let voice ← voice.mapM (·.toModal)
  let rooms ← rooms.mapM fun room => room.mapM (·.toModal)
  let anchor ← anchorSig
  let clock ← clockLit
  let mut nodes : Array PatchNode := #[{
    id := "src", node := .modalSource voice anchor clock none none none }]
  let mut input := "src"
  for (room, index) in rooms.zipIdx do
    let id := s!"rev{index}"
    nodes := nodes.push { id, node := .modalReverb input room none }
    input := id
  pure { nodes, output := input }

private def collectedTerm (voice : Array SeamMode)
    (rooms : Array (Array SeamMode)) : BuildM ArrowTerm := do
  let mut modes ← voice.mapM (·.toModal)
  for room in rooms do
    modes ← residueComposeEC modes (← room.mapM (·.toModal))
  pure <| modalBankTerm modes (← anchorSig) (← clockLit)
...
def runEcddPartition (arena : Arena)
  ...
  let gCold := modalChainGraph voice #[room1, roomCold]
  let directCold := collectedTerm voice #[room1, roomCold]
  ...
  let gHot1 := modalChainGraph voice #[roomHot]
  let gHotC := modalChainGraph voice #[room1, roomHot]
  ...
  let structural := freezeBuild arena do
    let vM ← voice.mapM (·.toModal)
    let r1M ← room1.mapM (·.toModal)
    let rHM ← roomHot.mapM (·.toModal)
    let r2CM ← r2Coin.mapM (·.toModal)
    let (plainS, pairedS) ← residueComposePartitioned vM rHM
    let (_, pairedC) ← foldRoomsPartitioned vM #[r1M, rHM]
    let (_, pairedR) ← foldRoomsPartitioned vM #[r1M, r2CM]
    ...
    pure (railRouted, railDiscr, refusedStated, structOk)
  let (railRouted, railDiscr, refusedStated, structOk) ← match structural with
    | .error error =>
        return ← failGate "ecdd-partition" s!"structural probe: {firstLine error}"
    | .ok (_, checks) => pure checks
  let lensOk := railRouted && railDiscr && refusedStated
  let gCoinC := modalChainGraph voice #[room1, r2Coin]
  match ← renderGraph arena "ecdd_cold" gCold,
        ...
        ← renderTerm arena "ecdd_hot1_coll" (collectedTerm voice #[roomHot]) nWin,
        ← renderGraphN arena "ecdd_hotC" gHotC nWin,
        ← renderTerm arena "ecdd_hotC_coll" (collectedTerm voice #[room1, roomHot]) nWin,
...
private def tallyRoutes (n : Nat) (mk : Nat → BuildM (ModalMode × ModalMode)) :
    BuildM RouteTally := do
...
def runEcddSigmaAxis (arena : Arena)
    (_resolved : Array (String × ProgramIdx)) : IO Bool := do
  ...
  let bake := fun (om sigma amp : Float) => do
  ...
  let liveRoom := fun (om amp : Float) => do
    let numerator ← lit 691 2
    let sigma ← div numerator (← clampE (← litF 2.0) (← litF 0.2) (← litF 12.0))
    let omega ← litF om
    let cre ← litF amp
    let cim ← lit 0
    pure ({ sigma, omega, cre, cim, sigmaRange := some rtSpan } : ModalMode)
  -- ARM A — the SHIPPED topology: baked partials over the gong register box
  -- against a LIVE-rt60 room (σ declared over [0.576, 34.55])
  let armA := fun (i : Nat) => do
    let sV := 0.2 + 3.1 * haltonF 3 i
    let fV := 110.0 + 913.0 * haltonF 2 i
    let dOm := (haltonF 7 i - 0.5) * 1.6
    pure (← bake (tp * fV) sV (0.4 + 0.6 * haltonF 5 i),
  ...
  let checks := runBuild arena do
    let sepCold ← do pure ((← classifyCoupling (← pt 800.0 0.4 0.8)
      (← pt 800.0 1.0 0.6)) == .cold)
    let nearHot ← do pure ((← classifyCoupling (← pt 800.0 0.7 0.8)
      (← pt 800.0 1.0 0.6)) == .paired)
    let omegaHot ← do pure ((← classifyCoupling (← pt 800.0 0.6 0.9)
      (← bake (tp * 800.0 + 0.05) 0.6 0.8)) == .paired)
    let dipper ← do pure ((← classifyCoupling (← pt 220.0 3.455 1.0)
      (← liveRoom (tp * 220.0) 0.7)) == .paired)
    let outSpan ← do pure ((← classifyCoupling (← pt 220.0 0.05 1.0)
      (← liveRoom (tp * 220.0) 0.7)) == .cold)
    let exQ ← do pure ((← classifyCoupling (← pt 400.0 0.02 1.0)
      (← pt 400.0 1.0 1.0)) == .cold)
    pure (sepCold, nearHot, omegaHot, dipper, outSpan, exQ,
      ← tallyRoutes 1000 armA, ← tallyRoutes 1000 armB)
  let (sepCold, nearHot, omegaHot, dipper, outSpan, exQ, a, b) ←
    match checks with
    | .error error =>
        return ← failGate "ecdd-sigma-axis" s!"arena-native classifier: {firstLine error}"
    | .ok (_, values) => pure values
...
def runEcddLive (arena : Arena)
  ...
  let liveRoomE := fun (rv : Float) (fHz amp : Float)
      (range? : Option (Float × Float)) => do
    let sigma ← div (← lit 691 2)
      (← clampE (← litF rv) (← litF rtLo) (← litF rtHi))
    let omega ← litF (tp * fHz)
    let cre ← litF amp
    let cim ← lit 0
    pure #[({ sigma, omega, cre, cim, sigmaRange := range? } : ModalMode)]
  let span := some (6.91 / rtHi, 6.91 / rtLo)
  let graphAt : Float → Float → Float → Option (Float × Float) → Bool →
      BuildM PatchGraph := fun rv fHz amp range? chain => do
    let vM ← voice.mapM (·.toModal)
    let live ← liveRoomE rv fHz amp range?
    let anchor ← anchorSig
    let clock ← clockLit
    let source : PatchNode := ⟨"src", .modalSource vM anchor clock none none none⟩
    if chain then
      let baked ← room1.mapM (·.toModal)
      pure { nodes := #[source,
        { id := "rev0", node := .modalReverb "src" baked none },
        { id := "rev1", node := .modalReverb "rev0" live none }], output := "rev1" }
    else
      pure { nodes := #[source,
        { id := "rev0", node := .modalReverb "src" live none }], output := "rev0" }
  -- (1) structure: tuned+ranged routes; untuned stays cold; rangeless stays cold
  let structural := freezeBuild arena do
    let vM ← voice.mapM (·.toModal)
    let r1M ← room1.mapM (·.toModal)
    let tuned ← liveRoomE 2.0 220.0 0.7 span
    let (_, pTuned) ← residueComposePartitioned vM tuned
    let (_, pUntuned) ← residueComposePartitioned vM
      (← liveRoomE 2.0 500.0 0.7 span)
    let (_, pNoRange) ← residueComposePartitioned vM
      (← liveRoomE 2.0 220.0 0.7 none)
    let (_, pChain) ← foldRoomsPartitioned vM #[r1M, tuned]
    pure (pTuned.size == 1 && pUntuned.isEmpty && pNoRange.isEmpty &&
      pChain.size == 1)
  let structOk ← match structural with
    | .error error =>
        return ← failGate "ecdd-live" s!"structural probe: {firstLine error}"
    | .ok (_, value) => pure value
  ...
  let oobGraph : BuildM PatchGraph := do
    let vM ← voice.mapM (·.toModal)
    let sigma ← div (← lit 691 2)
      (← clampE (← litF 0.1) (← litF 0.05) (← litF 30.0))
    let omega ← litF (tp * 220)
    let cre ← litF 0.7
    let cim ← lit 0
    let mode : ModalMode := { sigma, omega, cre, cim, sigmaRange := span }
    let anchor ← anchorSig
    let clock ← clockLit
    pure { nodes := #[
      { id := "src", node := .modalSource vM anchor clock none none none },
      { id := "rev0", node := .modalReverb "src" #[mode] none }], output := "rev0" }
  match ← renderGraphN arena "ecddlive_a" (graphAt 2.0 220.0 0.7 span false) nWin,
        ← renderGraphN arena "ecddlive_b" (graphAt 4.0 220.0 0.7 span false) nWin,
        ← renderGraphN arena "ecddlive_chain" (graphAt 2.0 220.0 0.7 span true) nWin,
        ← renderGraphN arena "ecddlive_oob" oobGraph nWin with
...
def runEcddGauge (arena : Arena)
  ...
  let gaugedGraph := fun (room : Array SeamMode) => do
