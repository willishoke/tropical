import Tropical.Playground.ArenaVocabulary
import Tropical.Playground.Decode

/-!
# Playground.ArenaDecode
...
namespace Tropical.Playground.ArenaCompiler
...
def masterClock (pidx : String → Option Nat) : BuildM Clock := do
  let one ← lit 1
  let zero ← lit 0
  let velocity ← pref pidx Tropical.Playground.masterVelocityParam one
  let tauBase ← pref pidx Tropical.Playground.masterTauBaseParam zero
...
private def paramValue (pidx : String → Option Nat) (paramName : String → String)
  ...
  if Tropical.Playground.isGlided kind kname then
...
def buildNodeWithParamNames (pidx : String → Option Nat)
  ...
  let implicitFanIn : Bool → String × Array PatchNode := fun modal =>
    let inputs := Tropical.Playground.portSources inObj "in"
  ...
  let modalControl (kname : String) : BuildM ModalControlRef := do
    ...
    pure ({ fallback := fallback
            signalNode? := (Tropical.Playground.portSources inObj kname)[0]? } : ModalControlRef)
  match kind with
  | "knob" =>
  ...
  | "source" =>
    ...
    let pitchE ← match (Tropical.Playground.portSources inObj "freq")[0]? with
    ...
    let anchor : Option Anchor ← match pidx s!"{paramName "freq"}#phase" with
      ...
      | none => pure none
    match (Tropical.Playground.portSources inObj "pm")[0]? with
    | some modId =>
  ...
  | "sflange" =>
    let depth ← paramValue pidx paramName kind params "depth"
    match (Tropical.Playground.portSources inObj "mod")[0]? with
  ...
  | "mix" => pure (.mix (Tropical.Playground.portSources inObj "in"), #[])
  | "ring" => pure (.ring (Tropical.Playground.portSources inObj "in"), #[])
  | "resonator" =>
    let f0 ← paramValue pidx paramName kind params "freq"
    let decay ← paramValue pidx paramName kind params "decay"
    let npart := (Tropical.Playground.jInt params "partials" 6).toNat
    let address := (Tropical.Playground.portSources inObj "addr")[0]?
    let zero ← lit 0
    match Tropical.Playground.jNum? params "partials_max" with
    | none =>
      let modes ← resonatorBank f0 decay npart
      pure (.modalSource modes zero clk address, #[])
    | some _ =>
      let capacity := (Tropical.Playground.jInt params "partials_max" 6).toNat
  ...
  | "reverb" =>
    ...
    let rtRange := Tropical.Playground.displayRangeOf "reverb" "rt60"
  ...
  | "modal_allpass_tail" =>
    ...
    pure (.modalLinear modalInput (modalAllpassTailStage center sweep rate
      (Tropical.Playground.jFloat params "ratio" 1.0)), modalFanIn)
  | "modalblend" =>
    let dry := ((Tropical.Playground.portSources inObj "dry")[0]?).getD "__silence__"
    let wet := ((Tropical.Playground.portSources inObj "wet")[0]?).getD "__silence__"
    let mix ← modalControl "mix"
    pure (.modalBlend dry wet mix, #[])
  | "modalmix" => pure (.modalMix (Tropical.Playground.portSources inObj "in"), #[])
  | "gauge" =>
    let gauge ← modalControl "g"
    pure (.modalGaugeControl modalInput gauge, modalFanIn)
  | "gong" =>
    let anchorSeconds ← litOfD (Tropical.Playground.jExactD params "t" (0, 0))
    let sr ← sampleRate
    ...
    let (full, half) ← if fullFromJson.isEmpty && halfFromJson.isEmpty then
        defaultGongModes (Tropical.Playground.jFloat params "freq" 110.0)
      else pure (fullFromJson, halfFromJson)
    let beta ← paramValue pidx paramName kind params "beta"
    let settle ← litOfD (Tropical.Playground.jExactD params "g" (18, 1))
    pure (gongStrikeNodes id clk anchor beta settle full half)
  | "bloomgong" =>
    let beta := Tropical.Playground.jFloat params "beta" 0.05
    let settle := Tropical.Playground.jFloat params "g" 1.8
    let scale := Tropical.Playground.jFloat params "scale" 1.0
    let decoded ← jModes params "modes"
    let modes ← if decoded.isEmpty then do
        let (full, _) ← defaultGongModes (Tropical.Playground.jFloat params "freq" 110.0)
        pure full
      else pure decoded
    let anchorSeconds ← litOfD (Tropical.Playground.jExactD params "t" (0, 0))
    let sr ← sampleRate
  ...
  | "string" =>
    ...
    let modes ← if decoded.isEmpty then
        defaultStringModes
          (Tropical.Playground.jDec params "freq" (196, 0))
          (Tropical.Playground.jDec params "decay" (996, 3))
      else pure decoded
    let anchorSeconds ← litOfD (Tropical.Playground.jExactD params "t" (0, 0))
    let sr ← sampleRate
    let anchor ← mul anchorSeconds sr
  ...
  | _ => pure (.mix (Tropical.Playground.portSources inObj "in"), #[])
...
def decodeGraph (j : Json) (params : Array (String × JsonNumber)) : BuildM PatchGraph := do
  let outId := match (j.getObjVal? "out").toOption with
    ...
    | _ => ""
  let raws := Tropical.Playground.rawsOf j
  let pidx : String → Option Nat := fun name => params.findIdx? (·.1 == name)
  let mut nodes : Array PatchNode := #[]
  for raw in raws do
    if raw.kind == "out" then continue
    let (node, extras) ← buildNodeWithParamNames pidx (Tropical.Playground.paramNameOf raw)
  ...
  let outputInputs := match raws.find? (·.id == outId) with
    | some raw => if raw.kind == "out" then
        Tropical.Playground.portSources raw.inObj "in" else #[raw.id]
    | none => #[]
  let masterIdx := (pidx Tropical.Playground.masterGainParam).getD 0
...
end Tropical.Playground.ArenaCompiler
