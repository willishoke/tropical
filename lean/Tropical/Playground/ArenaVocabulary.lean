...
/-!
# Playground.ArenaVocabulary

Arena-native scalar and modal builders for the production playground decoder.
The public vocabulary tables remain pure in `Playground.Vocabulary`; this file
only owns constructions that allocate expression nodes.
-/
...
def jExpr (obj : Json) (key : String) (dflt : Sig) : BuildM Sig :=
  match Tropical.Playground.jNum? obj key with
...
def lnEightyLit : BuildM Sig := litOfD Tropical.Playground.ln80D
...
def jModes (obj : Json) (key : String) : BuildM (Array ModalMode) := do
  ...
  for row in rows do
    ...
    let omega ← litOfD (DyadicI.mul Tropical.Playground.twoPiD (numD 0))
...
def resonatorBank (f0 decay : Sig) (npart : Nat) : BuildM (Array ModalMode) :=
  (Array.range npart).mapM fun j => do
    ...
    let sigFac := DyadicI.add DyadicI.one
      (DyadicI.mul (Tropical.Playground.decD (4, 1)) kD)
    let ampD := DyadicI.div DyadicI.one
      (DyadicI.pow kD (Tropical.Playground.decD (11, 1)))
...
def defaultStringModes (f0 rho : Int × Nat) : BuildM (Array ModalMode) := do
  ...
  let rhoD := Tropical.Playground.decD rho
  ...
  for j in [0:kmax] do
    ...
    let gain := DyadicI.mul rhoD (DyadicI.cos
      (DyadicI.div (DyadicI.mul Tropical.Playground.piD k) span))
    if DyadicI.certLt frequency halfSR && DyadicI.certGt gain DyadicI.zero then
      ...
      let phase := DyadicI.mul Tropical.Playground.goldenAngleD k
      let sigma ← litOfD sigmaD
      let omega ← litOfD (DyadicI.mul Tropical.Playground.twoPiD frequency)
      let cre ← litOfD (DyadicI.mul amplitude (DyadicI.cos phase))
      let cim ← litOfD (DyadicI.mul amplitude (DyadicI.sin phase))
      modes := modes.push { sigma, omega, cre, cim }
  pure modes
...
def reverbRoom (rt60 : Sig) (rtRange : Option (Float × Float))
  ...
  let c691 : DyadicI := Tropical.Playground.decD (691, 2)
  ...
  let floD := Tropical.Playground.decD flo
  let fhiD := Tropical.Playground.decD fhi
  ...
  (Array.range nmode).mapM fun j => do
    ...
    let phase := DyadicI.mul Tropical.Playground.twoPiD
      (DyadicI.mul Tropical.Playground.goldenRatioD (DyadicI.ofNat j))
...
def fallbackOf (kind kname : String) : BuildM Sig :=
  match (Tropical.Playground.portOf kind kname).bind (·.knob) with
