...
Status: **Phase 5 complete — passes AND the production seam are Lean**
(the plan-level reference interpreter is the remaining deferred item —
see phase 2 notes). Lean owns the front half of the compiler AND the
full strata pipeline: raise (`Tropical/Parse/Raise.lean` over the typed
AST in `Tropical/Parse/Nodes.lean`), the elaborator
...
codec, all six strata passes (`Tropical/Ir/Strata/*`, gated by
`make diff-strata` at K=5: pure Lean-vs-TS over the corpus, both wire
modes), AND every production strata call site (stage 6b): the engine
relinks + stratas registrations (`strataConcrete`), specializes
generics engine-side (`Tropical/TypeArgs.lean` + `runStrataChecked`,
cached per `Type<N=8>` key), and lifts array-literal wires itself
(`Tropical/Ir/WireProgram.lean`). The engine elaborates registered
programs AND the session root itself; the TS compiler service performs
no raise, no elaboration, and no strata on any production path —
`register_program` is typeDef-registration + decode + `makeCompiled`,
`resolve_type`/`register_lifted` are decode + registry residue,
`compile` instantiates per-instance shipped resolved + runs
`partitionKernel`. The stdlib
...
order. Gates: the Phase-1–3 battery (`diff-engine` 7 scripts / 113
steps incl. `debug_render` audio probes — `generics.json` and
`lifted_wires.json` added at 6b to discriminate the moved call sites,
`test-lean-engine` 67 tests,
