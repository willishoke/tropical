...
| DR-20 | **implemented visual correction** | Use one analytic fixed volts/div scale for all five projected fundamentals, select the freshest valid crossing, suppress un-lockable silent/DC traces, and request stride-1 samples. | Raw production-socket probes showed every backend channel was dynamic; per-frame autoscaling had erased the modal envelope and 5× decimation had introduced interpolation shimmer. The clean 15.38-second integrated smoke measured scope p99 `4.391 ms`, `24.00 fps`, 521 control writes, and zero faults. |
