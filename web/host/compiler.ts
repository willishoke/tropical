...
/** Parse a `tropical_plan_5` JSON string (e.g. fetched from
 *  /patches/foo.plan.json) and compile it. The JSON shape is
 *  `WireFlatPlan`; `parseWirePlan` resurrects the branded internal
 *  shape so `emitWasm` consumes a typed `FlatPlan`. */
export async function compilePlanJson(json: string, maxBlockSize = 2048): Promise<LoadedPlan> {
  ...
  return compilePlan(parseWirePlan(wire), maxBlockSize)
}
