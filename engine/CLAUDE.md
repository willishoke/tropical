...
1. `NumericProgramParser::parse_plan5()` — thin deserializer, reads per-instance plans plus the scheduler into a `FlatProgram` (multi-function) struct. A backcompat lift exists for single-kernel `plan_4` inputs (hand-crafted unit tests, legacy saved plans) — they parse into a one-instance plan_5.
2. `OrcJitEngine::compile_flat_program()` — JIT compiles the FlatProgram to a single native kernel function whose body is the scheduler.
