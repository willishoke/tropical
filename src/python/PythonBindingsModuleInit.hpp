...
PYBIND11_MODULE(egress, m)
{
  ...
  py::class_<PythonGraph>(m, "Graph")
    ...
    .def("prime_numeric_jit", &PythonGraph::prime_numeric_jit)
    .def("set_worker_count", &PythonGraph::set_worker_count, py::arg("worker_count"))
    .def("set_fusion_enabled", &PythonGraph::set_fusion_enabled, py::arg("enabled"))
    .def("fusion_enabled", &PythonGraph::fusion_enabled)
    .def("worker_count", &PythonGraph::worker_count)
