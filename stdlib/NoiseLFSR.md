...
program NoiseLFSR(clock = 0) -> (out: signal) {
  ...
  out = let {
      tick: (clock > 0.5) * (prev_clock <= 0.5);
      lsb: state & 1;
      shifted: state >> 1
    } in let {
      newState: select(lsb, shifted ^ 46080, shifted);
      normalized: select(lsb, shifted ^ 46080, shifted) * 2 / 65535 - 1
    } in select(tick, normalized, value)
  next state = let {
      tick: (clock > 0.5) * (prev_clock <= 0.5);
      lsb: state & 1;
      shifted: state >> 1
