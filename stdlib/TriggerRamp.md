...
program TriggerRamp(trigger: signal = 0) -> (frames: float, edge: float) {
  ...
  delay state: RampState = match state {
      Quiescent => select(trigger > 0.5 && prev_trigger <= 0.5,
        Counting { n: 0 }, Quiescent { }),
      Counting { n: n } => select(trigger > 0.5 && prev_trigger <= 0.5,
        Counting { n: 0 }, Counting { n: n + 1 })
    } init Quiescent { }
