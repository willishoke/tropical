...
program Log(x: float = 1) -> (out: float) {
  out = let {
      safe_x: select(x <= 0, 1e-45, x);
      e0: floatExponent(safe_x);
      m0: ldexp(safe_x, -e0);
      is_big: m0 > 1.4142135623730951;
      e: e0 + select(is_big, 1, 0);
      m: select(is_big, m0 * 0.5, m0);
      f: m - 1;
      poly: fold([0.06666666666666667, -0.07142857142857142,
        0.07692307692307693, -0.08333333333333333, 0.09090909090909091,
        -0.1, 0.1111111111111111, -0.125, 0.14285714285714285,
        -0.16666666666666666, 0.2, -0.25, 0.3333333333333333, -0.5, 1],
        0, (a, c) => c + a * f)
    } in f * poly + e * 0.6931471805599453
}
```
