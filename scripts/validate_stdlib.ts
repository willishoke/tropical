...
function validateTarget(patchPath: string): Result {
  ...
  // Owned eagerly so the `finally` always disposes — leaking a
  // runtime per iteration (the previous behavior) accumulates FFI
  // handles, and Bun's GC/FinalizationRegistry tripping over them
  // mid-run produces a hard segfault on shutdown.
